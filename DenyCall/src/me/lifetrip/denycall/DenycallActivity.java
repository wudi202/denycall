...
public class DenycallActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        //ii.addAction(Intent.ACTION_CALL_BUTTON);
        //ii.addAction("android.provider.Telephony.SMS_RECEIVE");
        //registerReceiver(new DenyCallBroadCastRecv(), new IntentFilter(Intent.ACTION_ANSWER));
		if (false == DenyCallService.isServiceStarted)
		{
			try {
	            Intent service = new Intent(this, DenyCallService.class);
	            startService(service);
			}
			catch (Exception e)
			{
				Log.e("create service fail", e.toString());
			}
		}        
        }
        catch (Exception e)
		...
		try
		{
			switch (state)
			{
			    ...
			    case TelephonyManager.CALL_STATE_RINGING: //响铃
			    {
			    	//看电话是否是黑/白名单
			    	//正常来讲白名单的概率比较大一些
			    	if (isPhoneInContacts(incomingNumber))//看来电是否在电话本里面
			    	{
			    		break;
			    	}				    	
			    	
			    	ContentResolver CurResolver = context.getContentResolver();
			    	//是否有前面的+86的问题？
			    	...
			    	//这里表示是否在禁止列表里面的有对应的区域号码，如：13810686793==》13810680000
			    	else
			    	{
			    		String incomingSub = incomingNumber.substring(0, incomingNumber.length()-4) + "0000";
			    		selection = FilterListMetaData.PHONE_NUMBER + "=" + "\"" + incomingSub + "\"";
			    		cFilterPhone = CurResolver.query(FilterListMetaData.CONTENT_URI, null, selection, null, FilterListMetaData.DEFAULT_SORT_ORDER);
			    		if (0 != cFilterPhone.getCount())
			    		{
				    	    cFilterPhone.moveToFirst();
				    	    int iColumnIndex = cFilterPhone.getColumnIndex(FilterListMetaData.IS_BLACK);
				    	    int iBlack = cFilterPhone.getInt(iColumnIndex);
    ...
    private Cursor getContacts()
    {
    	...
    	if (cursor.moveToFirst())
    	{
    		...
    		while (!cursor.isAfterLast())
    		{
    	        ...
    	        phonesNumber = context.getContentResolver().query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI,   
